...
function checkTableName($shortTName, $type=false)
{
	...
	if ("petugas" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("produk" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("detailpenjualan" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("penjualan" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("pelanggan" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	return false;
}
...
function AfterFBLogIn( $pUsername, $pPassword, $pDisplayUsername, &$pageObject = null)
{
	...
	$strSQL = "select * from ".$connection->addTableWrappers("petugas")
